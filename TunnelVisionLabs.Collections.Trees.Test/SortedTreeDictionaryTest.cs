...
namespace TunnelVisionLabs.Collections.Trees.Test
{
    ...
    public class SortedTreeDictionaryTest
    {
        [Fact]
        public void TestSortedTreeDictionaryConstructor()
        {
            SortedTreeDictionary<int, int> dictionary = new SortedTreeDictionary<int, int>();
        ...
        {
            Assert.Throws<ArgumentNullException>("collection", () => new SortedTreeDictionary<int, int>(collection: null));
            Assert.Throws<ArgumentNullException>("collection", () => new SortedTreeDictionary<int, int>(branchingFactor: 4, collection: null, comparer: null));

            Assert.Throws<ArgumentException>(() => new SortedTreeDictionary<int, int>(new[] { new KeyValuePair<int, int>(1, 1), new KeyValuePair<int, int>(1, 2) }));
            Assert.Throws<ArgumentException>(() => new SortedTreeDictionary<int, int>(branchingFactor: 4, new[] { new KeyValuePair<int, int>(1, 1), new KeyValuePair<int, int>(1, 2) }, comparer: null));

            var dictionary = new SortedTreeDictionary<int, int>(new[] { new KeyValuePair<int, int>(1, 1) });
            Assert.Single(dictionary);
            Assert.Equal(new[] { new KeyValuePair<int, int>(1, 1) }, dictionary);

            dictionary = new SortedTreeDictionary<int, int>(branchingFactor: 4, new[] { new KeyValuePair<int, int>(1, 1) }, comparer: null);
            Assert.Single(dictionary);
            Assert.Equal(new[] { new KeyValuePair<int, int>(1, 1) }, dictionary);
        }
        ...
        {
            var dictionary = new SortedTreeDictionary<int, int>();
            SortedTreeDictionary<int, int>.KeyCollection keys = dictionary.Keys;
            SortedTreeDictionary<int, int>.ValueCollection values = dictionary.Values;
...
#pragma warning restore xUnit2017 // Do not use Contains() to check if a value exists in a collection
        ...
        {
            IDictionary<int, int> dictionary = new SortedTreeDictionary<int, int>(branchingFactor: 4);
            for (int i = 0; i < 10; i++)
                dictionary.Add(i, i);
        ...
        {
            SortedTreeDictionary<int, int> set = new SortedTreeDictionary<int, int>(8);
            Assert.Empty(set);

            Assert.Throws<ArgumentOutOfRangeException>(() => new SortedTreeDictionary<int, int>(-1));
            Assert.Throws<ArgumentOutOfRangeException>(() => new SortedTreeDictionary<int, int>(0));
            Assert.Throws<ArgumentOutOfRangeException>(() => new SortedTreeDictionary<int, int>(1));
        }
        ...
        {
            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>().Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>().Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>().Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(Enumerable.Empty<KeyValuePair<object, object>>()).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(Enumerable.Empty<KeyValuePair<int, int>>()).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(Enumerable.Empty<KeyValuePair<IComparable, IComparable>>()).Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(comparer: null).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(comparer: null).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(comparer: null).Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(Enumerable.Empty<KeyValuePair<object, object>>(), comparer: null).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(Enumerable.Empty<KeyValuePair<int, int>>(), comparer: null).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(Enumerable.Empty<KeyValuePair<IComparable, IComparable>>(), comparer: null).Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(4).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(4).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(4).Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(4, comparer: null).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(4, comparer: null).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(4, comparer: null).Comparer);

            Assert.Same(Comparer<object>.Default, new SortedTreeDictionary<object, object>(4, Enumerable.Empty<KeyValuePair<object, object>>(), comparer: null).Comparer);
            Assert.Same(Comparer<int>.Default, new SortedTreeDictionary<int, int>(4, Enumerable.Empty<KeyValuePair<int, int>>(), comparer: null).Comparer);
            Assert.Same(Comparer<IComparable>.Default, new SortedTreeDictionary<IComparable, IComparable>(4, Enumerable.Empty<KeyValuePair<IComparable, IComparable>>(), comparer: null).Comparer);
        }
        ...
        {
            var dictionary = new SortedTreeDictionary<int, int>(branchingFactor: 4);
            for (int i = 0; i < 100; i++)
                dictionary.Add(i, i + 1);

            for (int i = 0; i < 100; i++)
            {
                Assert.Equal(i, dictionary.IndexOfKey(i));
                Assert.Equal(i, dictionary.IndexOfValue(i + 1));
            }
        }
    }
}
